...
BUILD_VERSION = "V7-2026-09-03"
FORM_FILENAME = "WoA_SAC_Application_2026-09-03.pdf"
...
def official_sac_form():
    response = send_file(
        BASE_DIR / "static" / "forms" / FORM_FILENAME,
    ...
    response.headers["X-ACE-Build"] = BUILD_VERSION
    return response


@app.route("/application")
def application_page():
    response = app.make_response(render_template("application.html", active="application", build_version=BUILD_VERSION))
    response.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    response.headers["Pragma"] = "no-cache"
    response.headers["X-ACE-Build"] = BUILD_VERSION
    return response
...
@app.get("/build-info")
def build_info():
    return jsonify({"build": BUILD_VERSION, "form": FORM_FILENAME})
